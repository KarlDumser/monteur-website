...
export default function BookingEditor({ booking, auth, onClose, onSave }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name.includes('Date') ? value : name.includes('people') || name.includes('Price') || name.includes('discount') || name.includes('vat') || name.includes('total') || name.includes('cleaningFee') || name.includes('nights') ? (isNaN(value) ? value : Number(value)) : value
    }));
  };
