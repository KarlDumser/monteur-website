...
router.get('/inquiries/email-diagnostics', async (req, res) => {
  try {
    const result = await getInquiryEmailDiagnostics();
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.patch('/inquiries/:id/approve', async (req, res) => {
  try {
    ...
    sendBookingConfirmation(inquiry, 'confirmation')
      .then((emailResult) => {
